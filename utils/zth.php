...
if(! defined('FILE_CACHE_SUFFIX') ) 		define ('FILE_CACHE_SUFFIX', '.zth.txt');			// What to put at the end of all files in the cache directory so we can identify them
if(! defined('FILE_CACHE_PREFIX') ) 		define ('FILE_CACHE_PREFIX', 'zth');				// What to put at the beg of all files in the cache directory so we can identify them
if(! defined('FILE_CACHE_DIRECTORY') ) 		define ('FILE_CACHE_DIRECTORY', '../pub/zth');				// Directory where images are cached. Left blank it will use the system temporary directory (which is better for security)
...
class timthumb {
	...
	protected function sendImageHeaders($mimeType, $dataSize){
		...
		// RJ EDIT : MODIFY DOWNLOAD NAME
		// $ext = $this->getImgExt($mimeType);
		// header ('Content-Disposition: attachment; filename=zth_'.md5(date('i:s')).$ext);
		// END OF EDIT : MODIFY DOWNLOAD NAME
	...
	// RJ UPDATE : GET IMG EXT VIA MIME
	protected function getImgExt($mimeType){
		switch ($mimeType) {
			case 'image/jpeg':
				$ext = '.jpg';
				break;
	...
	// END OF UPDATE : GET IMG EXT VIA MIME
